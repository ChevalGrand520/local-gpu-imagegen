...
E1 已按冻结 §5.1 修正。同协议 B2/W3 故障对照已在 `docs/research/dsn-paired-fault-protocol.md` 中完成设计复审，但尚未实现或执行。后续唯一工作单元是按该方案先实现 localhost 响应丢失 fixture、共同 runner、语义测试和拒绝覆盖的结果写入；正常 Windows pilot 重跑不直接回答 W3 的异常路径效果。当前没有足够证据把项目描述为“已实现未知 job 自动恢复”或“已证明可靠性净改善”。
