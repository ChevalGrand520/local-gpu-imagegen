...
class ReleaseCandidateWheelTests(unittest.TestCase):
    ...
    def make_wheel(
        ...
        *,
        ...
        wheel = root / "local_gpu_imagegen-0.9.1-py3-none-any.whl"
        metadata = metadata or (
            "Metadata-Version: 2.4\nName: local-gpu-imagegen\nVersion: 0.9.1\n"
            "Requires-Python: >=3.11\nRequires-Dist: py7zr==1.1.3\n\n"
        )
        with zipfile.ZipFile(wheel, "w") as archive:
            archive.writestr("local_gpu_imagegen/__init__.py", '__version__ = "0.9.1"\n')
            if include_metadata:
                if metadata_mode is None:
                    archive.writestr("local_gpu_imagegen-0.9.1.dist-info/METADATA", metadata)
                else:
                    info = zipfile.ZipInfo("local_gpu_imagegen-0.9.1.dist-info/METADATA")
                    info.external_attr = metadata_mode << 16
                    archive.writestr(info, metadata)
            archive.writestr(
                "local_gpu_imagegen-0.9.1.dist-info/WHEEL",
                wheel_headers,
            )
            archive.writestr("local_gpu_imagegen-0.9.1.dist-info/RECORD", "")
            if include_bootstrap_assets:
                archive.writestr(
                    "local_gpu_imagegen-0.9.1.data/data/share/local-gpu-imagegen/"
    ...
    def test_wheel_binds_hash_metadata_and_registry_descriptor(self) -> None:
        ...
        self.assertEqual(facts["version"], "0.9.1")
    ...
    def test_wheel_rejects_duplicate_members_and_malformed_wheel_headers(self) -> None:
        ...
        with zipfile.ZipFile(wheel, "a") as archive:
            with warnings.catch_warnings():
                ...
                archive.writestr(
                    "local_gpu_imagegen-0.9.1.dist-info/METADATA",
                    "Metadata-Version: 2.4\nName: local-gpu-imagegen\nVersion: 0.9.1\n"
                    "Requires-Python: >=3.11\n\n",
                )
        results, _ = checks.inspect_wheel(root, wheel, self.sha(wheel))
...
class ReleaseCandidateInstalledTests(unittest.TestCase):
    ...
    def test_real_pip_accepts_matching_wheel_digest_and_rejects_mismatch(self) -> None:
        ...
        with zipfile.ZipFile(wheel, "w") as archive:
            archive.writestr(
                "local_gpu_imagegen/__init__.py", '__version__ = "0.9.1"\n'
            )
            archive.writestr(
                "local_gpu_imagegen-0.9.1.dist-info/METADATA", metadata
            )
            archive.writestr(
                "local_gpu_imagegen-0.9.1.dist-info/WHEEL",
...
class ReleaseCandidateReportTests(unittest.TestCase):
    ...
    def test_status_is_passed_only_when_every_check_passes(self) -> None:
        ...
        with (
            ...
            patch.object(checks, "run_installed_checks", return_value=(installed_checks, {"version": "0.9.1"})),
        ):
            report = checks.validate_candidate(
                root=self.root,
                wheel=self.wheel,
